...
# df_unique = df.drop_duplicates(subset='anime_id', keep='first')
# df_unique.to_csv('anime_id_titles.csv', index=False)
# exit()

# ids = sorted(df['anime_id'].unique().astype(int))
...
if os.path.exists(MODEL_SAVE_PATH) and os.path.exists(DATASET_SAVE_PATH):
    ...
    print("No pre-trained model or dataset found.")
    exit(1)

# --- 3. Fetch Data for a New Unseen User ---
user_id_map, user_feature_map, item_id_map, item_feature_map = dataset.mapping()
internal_to_original_anime_id = {v: k for k, v in item_id_map.items()}

CLIENT_ID = "e005ada615ec70506c9151481298f234"
headers = {
...
if scores.size > 0:
    ...
    unseen_item_scores_pairs = [(item_id, score) for item_id, score in item_scores_pairs if item_id not in seen_item_internal_ids]
    ...
    for i, (item_internal_id, score) in enumerate(top_items_scores[:top_n]):
        ...
        title = titles_map.get(original_anime_id, f"Unknown Anime (ID: {original_anime_id})")
